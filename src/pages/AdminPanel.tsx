...
const AdminPanel = () => {
	...
	const saveEvent = async (e: React.FormEvent, waiverFile?: File) => {
		...
		if (eventId) {
			...
			const originalEvent = events.find(
				(ev) => String(ev.id) === eventId
			);
			if (
				originalEvent &&
				originalEvent.date < now &&
				formattedDate >= now
			) {
		...
		if (eventForm.waiver_required && waiverFile) {
			...
			if (error)
				return alert("Failed to upload waiver PDF: " + error.message);
		...
		if (eventId) {
			...
			if (!data || data.length === 0)
				alert(
					"No event was updated. Check if the event ID matches the database."
				);
		} else {
	...
	return (
		<div className="max-w-7xl mx-auto px-4 py-12 space-y-10 mt-24">
			<h1 className="text-4xl font-bold text-[#8a9663] text-center mb-6">
				Admin Panel
			</h1>
