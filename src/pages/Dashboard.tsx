...
const Dashboard = () => {
	...
	useEffect(() => {
		const fetchData = async () => {
			...
			setTotalHours(
				(hoursData || []).reduce((sum, h) => sum + (h.hours || 0), 0)
			);
		...
		hoursArr.forEach((h) => {
			...
			if (period === "week") {
				...
				const week = Math.ceil(
					(((d as any) - (onejan as any)) / 86400000 +
						onejan.getDay() +
						1) /
						7
				);
	...
	if (loading)
		return (
			<div className="py-32 text-center text-xl text-[#4d5640]">
				Loading dashboard...
			</div>
		);
			...
			<div className="w-full max-w-screen-xl flex flex-col lg:flex-row gap-12">
				...
				<div className="space-y-10 w-full lg:w-1/2">
					<div className="bg-white rounded-3xl p-10 shadow-xl border border-[#b8c19a]">
						...
						<p className="text-lg text-[#6a7c3f] font-medium">
							You're on a mission to save the 🌍 — or at least the
							local park. Here's your impact.
						</p>
					</div>

					<div className="bg-white rounded-3xl p-8 shadow-xl border border-[#b8c19a]">
						<div className="flex items-center justify-between mb-4">
							<h2 className="text-2xl font-bold text-[#49682d]">
								Hours Volunteered
							</h2>
							<select
								...
								value={graphPeriod}
								onChange={(e) =>
									setGraphPeriod(e.target.value as any)
								}
							>
				...
				<div className="space-y-10 w-full lg:w-1/2">
					<div className="bg-white rounded-3xl p-8 shadow-lg border border-[#b8c19a]">
						<h2 className="text-2xl font-bold text-[#49682d] mb-6">
							Upcoming Events
						</h2>
						<ul className="space-y-4">
							{upcomingEvents.map((event) => {
								...
								return (
									...
									>
										<div>
											<p className="font-bold text-[#4a612c] text-xl">
												{event.title}
											</p>
					...
					<div className="bg-white rounded-3xl p-6 shadow-xl border border-[#b8c19a]">
						<h2 className="text-xl font-semibold text-[#49682d] mb-3">
							Past Events
						</h2>
				...
				<div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
					<div className="bg-white w-[90vw] sm:w-full max-w-2xl max-h-[85vh] overflow-y-auto rounded-3xl p-6 sm:p-8 relative shadow-2xl border border-[#b8c19a]">
						...
						<p className="text-sm font-semibold">
							Waiver Required:{" "}
							{selectedEvent.waiver_required ? "Yes" : "No"}
						</p>
