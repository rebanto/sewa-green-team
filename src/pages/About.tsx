		...
		<div className="overflow-x-hidden mt-30">
			...
			>
				...
				<motion.p
					...
					custom={1}
				>
					The Sewa Green Team (SGT) is a student-powered
					sustainability force, born in 2022 to make community events
					cleaner, greener, and more impactful.
				</motion.p>
			</motion.section>
						...
						{timelineData.map((item, index) => (
							<motion.div
								...
								className={`relative w-full md:w-1/2 px-6 py-6 rounded-xl bg-white/40 backdrop-blur-md shadow-md border border-[#cdd1bc] transition-all hover:scale-105 ${
									index % 2 === 0
										? "ml-auto -translate-x-5"
										: "mr-auto translate-x-5"
								}`}
								initial="hidden"
								whileInView="visible"
								viewport={{ once: true }}
					...
					<div className="flex flex-wrap justify-center gap-10 max-w-6xl mx-auto">
						{leadership.map((leader, index) => (
							...
							>
								...
								<h3 className="text-xl font-semibold text-[#8a9663] mb-1">
									{leader.role}
								</h3>
								<p className="text-sm text-[#858d6a] italic">
									{leader.name}
								</p>
							</motion.div>
						))}
